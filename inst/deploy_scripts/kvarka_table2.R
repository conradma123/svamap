...
names(table_kvarka) <- c("Antal påvisade kvarkaprover", "Antal kommuner")
## table_kvarka <- structure(list(`Antal påvisade kvarkaprover` = c("Ett tillfälle under de senaste 2 månaderna",
##                                                        "Två tillfällen under de senaste 2 månaderna",
##                                                        "Tre eller fler tillfällen under de senaste 2 månaderna",
##                                                        "Minst 1 tillfälle under senaste 3-24 månaderna"),
##                      `Antal kommuner` = c(16, 3, 1, 73)),
##                 .Names = c("Antal påvisade kvarkaprover", "Antal kommuner"),
##                 row.names = c(NA, -4L),
##                 class = "data.frame")
## Kvarka table
...
head <- hlt::html_head(hlt::html_meta(charset="utf-8") +
    ...
    hlt::html_style(c("  /**/",
                      ...
                      "  }")))
##
table <- hlt::html_table(table_kvarka, tfoot = FALSE)
...
for(i in seq_len(length(table$content[[2]]$content))) {
    hlt::tag_attr(table$content[[2]]$content[[i]]$content[[2]]) <- list(align = "right")
}
## Add attributes to the rows in the body
hlt::tag_attr(table$content[[2]]$content[[1]]) <- list(style = "background-color: #FED98E;")
hlt::tag_attr(table$content[[2]]$content[[2]]) <- list(style = "background-color: #FE9929;")
hlt::tag_attr(table$content[[2]]$content[[3]]) <- list(style = "background-color: #CC4C02;")
hlt::tag_attr(table$content[[2]]$content[[4]]) <- list(style = "background-color: #FFFFD4;")
## Build the table
page <- hlt::html_html(
...
ftpUpload(tab, paste0(cred, "kvarka_table2.html"))
